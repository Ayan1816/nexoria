...
import {
  ArrowRight, Bot, Clock3, Cpu, Gauge, Orbit, ShieldCheck, Sparkles,
  Zap, LogOut, Lock, Timer, Sun, Moon, History, PenTool, CheckCircle
} from 'lucide-react';
...
export default function App() {
  ...
  const handleAction = async () => {
    ...
    if (activeModule === 'escrow') {
      ...
      try {
        ...
        setTimeout(async () => { fetchLockStatus(); await updateBalances(activeProvider, walletAddress); }, 4000);
  ...
  const handleClaim = async (index) => {
    ...
    try {
      ...
      setTimeout(async () => { fetchLockStatus(); await updateBalances(activeProvider, walletAddress); }, 5000);
    ...
    try {
      ...
      setSignature(sig); setIsExecuting(false);
  ...
  const handleAiCommand = async (cmd) => {
    if (!cmd) return;
    setAiLogs(prev => [...prev, { role: 'user', msg: cmd }]); setAiCommand(''); setIsAiProcessing(true);
    const match = cmd.match(/(?:send|transfer|route)\s+([\d.]+)\s*(usdc|eurc)?\s+(?:to\s+)?(0x[a-fA-F0-9]{40})/i);
    if (match) {
      const amountStr = match[1]; const tokenSymbol = match[2] ? match[2].toUpperCase() : 'USDC'; const toAddress = match[3];
      ...
      if (!walletAddress || !activeProvider) { setIsAiProcessing(false); setAiLogs(prev => [...prev, { role: 'system', msg: `ERROR: Wallet not connected.` }]); return; }
      try {
        let txHashRes;
        if (tokenSymbol === 'USDC') {
          ...
          txHashRes = (await contract.transfer(toAddress, window.ethers.parseUnits(amountStr, decimals))).hash;
        }
        ...
        setTimeout(async () => { await updateBalances(activeProvider, walletAddress); }, 5000);
      } catch(e) { setAiLogs(prev => [...prev, { role: 'system', msg: `ERROR: Execution failed.` }]); }
    } else {
       setTimeout(() => { setAiLogs(prev => [...prev, { role: 'ai', msg: `⚡ Intent received: "${cmd}". (Note: For REAL transfer, type "Send [amount] [USDC/EURC] to [0xAddress]")` }]); }, 1500);
    }
  ...
  const inputBg = isDark ? 'bg-slate-950 border-white/10 text-white' : 'bg-slate-50 border-slate-300 text-slate-900';
    ...
    <div className={`min-h-screen font-sans selection:bg-cyan-500/30 transition-colors duration-500 relative ${bgMain}`}>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        {/* 🔥 Your beautiful Hero & Balance Section 🔥 */}
        <section className={`border rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-8 justify-between items-start relative overflow-hidden transition-colors duration-500 ${bgCard}`}>
          ...
          <div className="space-y-4 relative z-10">
            <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-cyan-500 font-mono"><Cpu className="w-3 h-3" /> ArcOS - Agentic Economic Matrix</div>
